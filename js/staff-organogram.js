(function () {
  ...
  const personCard = (person, isHead, extraClass='') => `<button type="button" class="team-member-card ${isHead?'team-member-head':''} ${extraClass}" data-person='${esc(JSON.stringify(person))}'><span class="team-member-photo">${photoMarkup(person)}<span class="team-member-open">View profile</span></span><span class="team-member-label">${isHead?'<small>HEAD OF DEPARTMENT</small>':''}<strong>${esc(cleanName(person.name))}</strong><em>${esc(person.role)}</em></span></button>`;
  ...
  const leadershipPanel = data => {
    ...
    return `<div class="executive-hierarchy">
      ${founder ? `<div class="executive-level executive-founder-level">${personCard(founder,true,'executive-chairman-card')}</div>` : ''}
      <div class="executive-connector"></div>
      <div class="executive-level executive-senior-level">${rajesh ? personCard(rajesh,false,'executive-senior-card') : ''}${giri ? personCard(giri,false,'executive-senior-card') : ''}</div>
      <div class="executive-connector executive-connector-down"></div>
      ${rakesh ? `<div class="executive-level executive-fpic-level">${personCard(rakesh,true,'executive-fpic-card')}</div>` : ''}
      ${brijesh ? `<div class="executive-subordinate-line"></div><div class="executive-level executive-deputy-level"><div class="executive-under-label">REPORTING TO FPIC</div>${personCard(brijesh,false,'executive-deputy-card')}</div>` : ''}
    </div>`;
  };
  ...
  const build = data => {
    ...
    root.innerHTML = `<div class="team-intro"><p>Explore the leadership structure and departments of Dexin Manufacturing Nepal. Click a department or leadership level to discover individual profiles.</p></div><div class="team-department-grid">${departments.map((d,index)=>{
      if(d.leadership) return `<section class="team-department leadership-department" data-department><button type="button" class="department-toggle" aria-expanded="false"><span class="department-icon">L</span><span class="department-copy"><strong>Executive Leadership</strong><small>${d.count} leadership positions</small></span><span class="department-chevron" aria-hidden="true">+</span></button><div class="department-panel" hidden>${leadershipPanel(data)}</div></section>`;
      const count=d.groups.length?d.groups.reduce((n,g)=>n+1+(g.members||[]).length,0):(d.head?1:0)+d.members.length;
      return `<section class="team-department" data-department><button type="button" class="department-toggle" aria-expanded="false"><span class="department-icon">${esc(d.name.charAt(0))}</span><span class="department-copy"><strong>${esc(d.name)}</strong><small>${count} team member${count===1?'':'s'}</small></span><span class="department-chevron" aria-hidden="true">+</span></button><div class="department-panel" hidden>${d.groups.length?`<div class="team-unit-grid">${d.groups.map(g=>`<div class="team-unit"><div class="team-unit-title"><h3>${esc(g.name)}</h3><span>${1+(g.members||[]).length} members</span></div>${g.head?personCard(g.head,true):''}<div class="team-member-grid">${(g.members||[]).map(p=>personCard(p,false)).join('')}</div></div>`).join('')}</div>`:`<div class="team-member-grid department-members">${d.head?personCard(d.head,true):''}${d.members.map(p=>personCard(p,false)).join('')}</div>`}</div></section>`;
    }).join('')}</div><div class="team-profile-modal" data-profile-modal hidden><div class="team-profile-backdrop" data-profile-close></div><div class="team-profile-dialog" role="dialog" aria-modal="true" aria-labelledby="profile-name"><button type="button" class="team-profile-close" data-profile-close aria-label="Close profile">&times;</button><div class="team-profile-content" data-profile-content></div></div></div>`;
