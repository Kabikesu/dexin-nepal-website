(function () {
  ...
  const initials = name => String(name || '')
    .replace(/^Mr\.?\s|^Mrs\.?\s|^Ms\.?\s/, '')
    .split(/\s+/).filter(Boolean).map(x => x[0]).slice(0, 2).join('').toUpperCase();
  ...
  const personCard = (person, isHead) => `
    <button type="button" class="team-member-card ${isHead ? 'team-member-head' : ''}" data-person='${esc(JSON.stringify(person))}'>
  ...
  const build = data => {
    const leadership = {
      ...
      head: data.leadership?.[3] || null,
      members: data.leadership?.filter((_, i) => i !== 3) || [],
      groups: []
    };
    ...
    root.innerHTML = `
      <div class="team-intro">
        <p>Select a department to explore its team. Department heads are highlighted, and every profile opens in a detailed view.</p>
      </div>
      <div class="team-department-grid">
        ${departments.map((d, index) => {
          const count = d.groups.length
            ? d.groups.reduce((n, g) => n + 1 + (g.members || []).length, 0)
            : (d.head ? 1 : 0) + d.members.length;
          return `<section class="team-department ${index === 0 ? 'leadership-department' : ''}" data-department>
            ...
            <div class="department-panel" hidden>
              ${d.groups.length ? `<div class="team-unit-grid">${d.groups.map(g => `<div class="team-unit"><div class="team-unit-title"><h3>${esc(g.name)}</h3><span>${1 + (g.members || []).length} members</span></div>${g.head ? personCard(g.head, true) : ''}<div class="team-member-grid">${(g.members || []).map(p => personCard(p, false)).join('')}</div></div>`).join('')}</div>` : `<div class="team-member-grid department-members">${d.head ? personCard(d.head, true) : ''}${d.members.map(p => personCard(p, false)).join('')}</div>`}
            </div>
          </section>`;
        }).join('')}
      </div>
    ...
    root.querySelectorAll('.team-member-card').forEach(button => {
      button.addEventListener('click', () => {
        ...
        content.innerHTML = `
          <div class="profile-photo-large">${photoMarkup(person)}</div>
          <div class="profile-details">
            ${button.classList.contains('team-member-head') ? '<span class="profile-badge">HEAD OF DEPARTMENT</span>' : ''}
